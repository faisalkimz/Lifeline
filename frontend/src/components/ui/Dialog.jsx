...
const DialogContent = ({ children, className = "" }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className={`relative bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-200 animate-scale-in ${className}`}>
                {children}
            </div>
        </div>
    );
...
const DialogFooter = ({ children, className = "" }) => (
    <div className={`px-6 py-4 border-t border-slate-100 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-3 rounded-b-xl ${className}`}>
        {children}
    </div>
);
