    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Digital Forms & Checklists</h1>
                    <p className="text-gray-500 dark:text-gray-400">Manage internal requests and compliance forms</p>
                </div>
                {isAdmin && (
                    <button
                ...
                <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div className="flex bg-gray-100 rounded-lg p-1">
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'templates' ? 'bg-white dark:bg-slate-700 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-slate-800 dark:hover:text-slate-200'
                                }`}
                        >
                            Available Forms
                        </button>
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'submissions' ? 'bg-white dark:bg-slate-700 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-slate-800 dark:hover:text-slate-200'
                                }`}
                        >
                            Submissions
                        </button>
                                            ...
                                            {isAdmin && (
                                                <>
                                                    <button
                                                        ...
                                                        className="p-2.5 rounded-xl border border-gray-200 text-gray-400 hover:text-primary-600 hover:border-primary-600 transition-all"
                                                        title="Edit Template"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-2.5 rounded-xl border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-600 transition-all"
                            ...
                            <tbody>
                                {submissionsLoading ? (
                                    ...
                                    <tr key={sub.id} className="group">
                                        <td>
                                            <p className="font-medium text-gray-900 dark:text-white">{sub.template_details?.name}</p>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <button
                                                ...
                                                className="p-2 rounded-lg bg-gray-50 text-gray-400 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
                                            >
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">{isEditing ? 'Edit Form Template' : 'Create New Form Template'}</h2>
                            <button onClick={closeTemplateModal} className="text-gray-400 hover:text-gray-600"><XCircle className="h-5 w-5" /></button>
                        </div>
                        <form onSubmit={handleCreateTemplate} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div>
                                <label className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Form Name *</label>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center mb-1">
                                    <label className="text-sm font-bold text-slate-800">Form Fields</label>
                                    <button
                                        type="button"
                                        onClick={addField}
                                ...
                                <div className="space-y-3">
                                    {formFields.map((field, index) => (
                                        <div key={field.id} className="p-4 rounded-xl border-2 border-slate-100 bg-gray-50/50 dark:bg-gray-900/50 space-y-3 animate-in fade-in slide-in-from-top-2">
                                            <div className="flex justify-between items-center">
                                                <span className="text-[10px] font-black text-gray-400 uppercase">Field #{index + 1}</span>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                ...
                                                <div className="flex items-center gap-2 px-1">
                                                    <input
                                                        ...
                                                        id={`req-${field.id}`}
                                                    />
                                                    <label htmlFor={`req-${field.id}`} className="text-xs font-semibold text-gray-500">Required</label>
                                                </div>
                                            </div>
                                            {field.type === 'select' && (
                                                <input
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200">
                        ...
                        <form onSubmit={handleSubmitForm} className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                            {selectedTemplate.fields_config.map(field => (
                                <div key={field.name}>
                                    <label className="text-sm font-bold text-gray-700 dark:text-slate-300 mb-1.5 block">
            ...
            {isReviewModalOpen && selectedSubmission && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-left">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white">{selectedSubmission.template_details?.name}</h2>
                                <p className="text-sm text-gray-500">By {selectedSubmission.employee_details?.first_name} {selectedSubmission.employee_details?.last_name}</p>
                            </div>
                        ...
                        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                            <div className="grid grid-cols-1 gap-4">
                                {Object.entries(selectedSubmission.submission_data).map(([key, val]) => (
                                    <div key={key} className="bg-gray-50/40 p-4 rounded-xl border border-slate-100 shadow-sm">
                                        <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-1.5">{key.replace('_', ' ')}</p>
                                        <p className="text-gray-900 dark:text-white font-semibold text-lg">{val}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            {isAdmin && selectedSubmission.status === 'pending' && (
                                <form onSubmit={handleReviewSubmit} className="mt-8 pt-8 border-t border-slate-100 space-y-5">
                                    <div>
                                        <label className="text-sm font-bold text-gray-700 dark:text-slate-300 mb-2 block flex items-center gap-2">
                                        ...
                                        <textarea
                                            ...
                                            className="input-workpay bg-white"
                                        />
                                    </div>
                        ...
                        <div className="p-6 bg-gray-50/50 flex justify-end">
                            <button type="button" onClick={() => { setIsReviewModalOpen(false); setSelectedSubmission(null); }} className="btn-workpay-secondary bg-white dark:bg-gray-900">Close Window</button>
                        </div>
                    </div>
                </div>
            )}
