            ...
            {isLoading ? (
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <Card className="lg:col-span-2 overflow-hidden border-none shadow-xl ring-1 ring-slate-100">
                            <CardHeader className="bg-gray-50/50 border-b border-slate-100">
                            ...
                            <CardContent className="p-8">
                                <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                                    <div className="text-center md:text-left">
                                        <p className="text-xs font-bold text-gray-400 uppercase">Last Working Day</p>
                                    ...
                                    <div className="text-center md:text-right">
                                        <p className="text-xs font-bold text-gray-400 uppercase">Days Left</p>
                        ...
                        <div className="space-y-6">
                            <Card className="bg-gray-900 border-none shadow-2xl text-white relative overflow-hidden">
                                <div className="absolute -right-8 -bottom-8 opacity-10">
                                    <Shield className="h-40 w-40" />
                                </div>
                                <CardContent className="p-6 relative z-10">
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">System Access</h4>
                            ...
                            <Card className="bg-primary-600 border-none shadow-xl text-white">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-xl font-bold text-gray-900">Offboarding Workflow</h3>
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Departure steps</span>
                        </div>
                                ...
                                return (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            isDone
                                                ? "bg-primary-50 border-emerald-100"
                                                : "bg-white border-slate-100 hover:border-gray-200 shadow-sm"
                                        )}
                                    ...
                                    >
                                        <div className="flex items-start gap-4 h-full">
                                            <div className={cn(
                                                ...
                                                isDone ? "bg-primary-500 text-white rotate-12" : "bg-gray-100 text-gray-400"
                                            )}>
                                                {isDone ? <CheckCircle className="h-6 w-6" /> : <Icon className="h-6 w-6" />}
                                            </div>
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between">
                                                    <h4 className={cn("font-bold text-base", isDone ? "text-emerald-900" : "text-gray-900")}>
                                                        {step.title}
                                                    </h4>
                                                    {step.canToggle && (
                                                        <div className={cn(
                                                            "w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all",
                                                            isDone ? "bg-primary-500 border-emerald-500" : "border-gray-300"
                                                        )}>
                                                            {isDone && <CheckCircle className="h-3 w-3 text-white" />}
                                                        </div>
                                                    )}
                                                </div>
                                                <p className={cn("text-xs mt-2 leading-relaxed", isDone ? "text-primary-600" : "text-gray-500")}>
                                                    {step.description}
                                                </p>
                                                ...
                                                {step.id === 'exit_interview' && !isDone && (
                                                    <Button
                                                        variant="ghost"
                                                        className="mt-4 w-full h-8 text-[10px] uppercase font-black tracking-widest bg-gray-100 hover:bg-slate-200"
                ...
                <Card className="border-dashed border-4 border-slate-100 bg-gray-50/30">
                    <CardContent className="py-24 text-center max-w-2xl mx-auto">
                        ...
                        <h3 className="text-2xl font-black text-gray-900 tracking-tight">Standard Offboarding Procedure</h3>
            ...
            <Dialog open={isResignationDialogOpen} onOpenChange={setIsResignationDialogOpen}>
                <DialogContent className="max-w-2xl rounded-[2rem] border-none shadow-2xl p-0 overflow-hidden">
                    ...
                    <form onSubmit={handleSubmitResignation} className="p-8 space-y-6 bg-white">
                        ...
                        <div>
                            <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Reason for Leaving</label>
                            <select
                                value={resignationForm.reason}
                                onChange={(e) => setResignationForm({ ...resignationForm, reason: e.target.value })}
                                className="w-full h-12 px-4 rounded-2xl border-2 border-slate-100 bg-gray-50 text-gray-900 font-medium focus:ring-0 focus:border-rose-500 transition-all"
                                required
                            >
                        ...
                        <div>
                            <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Transition & Handover Notes</label>
                            <textarea
                                className="w-full min-h-[120px] p-4 rounded-2xl border-2 border-slate-100 bg-gray-50 text-gray-900 font-medium focus:ring-0 focus:border-rose-500 transition-all resize-none"
                    ...
                    <form onSubmit={handleSubmitExitInterview} className="p-8 space-y-8 bg-white">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700">Work-Life Balance Satisfaction</label>
                                <input
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700">Compensation Fairness</label>
                                <input
                                    type="range" min="1" max="10"
                                    className="w-full h-2 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                        ...
                        <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border-2 border-slate-100">
                            <input
                                ...
                                className="w-5 h-5 text-indigo-600 rounded-md border-gray-300"
                            />
                            <label htmlFor="recommend_ref" className="text-sm font-medium text-gray-700">
