    ...
    <div className="space-y-6 bg-white text-black p-6 rounded-lg">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleCreatePayrollRun} className="bg-gray-900 text-white font-bold px-6">
            New Payroll Run
          </Button>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row items-end gap-6">
            <div className="w-full md:w-48">
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-900 font-bold appearance-none cursor-pointer focus:ring-2 focus:ring-primary-500/20"
              >
            ...
            <div className="flex-1 flex justify-end">
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-600">Live Financial Matrix Active</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      {currentPayroll ? (
        <Card className="bg-white border rounded-2xl shadow-md border-gray-200 overflow-hidden">
          <CardHeader className="bg-gray-50/50 border-b border-slate-100 p-6">
          ...
          <CardContent className="p-8">
            {currentPayroll.employee_count === 0 ? (
              <div className="py-12 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 px-6">
                ...
                <p className="font-black text-gray-900 text-2xl tracking-tight">No Financial Data Processed</p>
                <p className="text-gray-500 text-lg max-w-md mx-auto mt-4 font-medium leading-relaxed">
                ...
                <div className="flex gap-4 justify-center mt-10">
                  <Button className="bg-gray-900 text-white font-bold h-12 px-8" onClick={() => navigate(`/payroll/runs/${currentPayroll.id}`)}>
                    Open Run Details
                  </Button>
                  <Button
                    variant="outline"
                    className="font-bold h-12 px-8 border-gray-300 hover:border-slate-900 hover:bg-gray-900 hover:text-white transition-all shadow-sm"
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div className="p-4 rounded-xl bg-gray-50/50">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Processing Status</p>
                  <p className="text-2xl font-black text-gray-900">{statusConfig[currentPayroll.status].label}</p>
                </div>
                <div className="p-4 rounded-xl bg-gray-50/50">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Total Participants</p>
                  <p className="text-2xl font-black text-gray-900">{currentPayroll.employee_count} Personnel</p>
                </div>
                <div className="p-4 rounded-xl bg-primary-50/50 border border-emerald-100">
                  <p className="text-xs font-black text-primary-600/50 uppercase tracking-widest mb-2">Company Obligation</p>
        ...
        <Card className="bg-white border text-center shadow-none rounded-2xl border-slate-100">
          <CardContent className="pt-24 pb-24">
            <div className="h-24 w-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-8 animate-pulse">
              <Calendar className="h-12 w-12 text-slate-300" />
            </div>
            <h2 className="text-3xl font-black text-gray-900 tracking-tighter">No Active Payroll Matrix</h2>
            <p className="text-gray-500 mb-10 mt-4 max-w-sm mx-auto text-xl font-medium leading-relaxed">System found no financial records for {new Date(selectedYear, selectedMonth - 1).toLocaleString('en-US', { month: 'long', year: 'numeric' })}.</p>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow>
                <TableHead className="font-black text-gray-900 h-14">Period</TableHead>
                <TableHead className="font-black text-gray-900">Status</TableHead>
                <TableHead className="text-right font-black text-gray-900">Units</TableHead>
                <TableHead className="text-right font-black text-gray-900">Gross Vol.</TableHead>
                <TableHead className="text-right font-black text-gray-900">Deductions</TableHead>
                <TableHead className="text-right font-black text-gray-900">Net Value</TableHead>
                <TableHead className="text-center font-black text-gray-900">Operations</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {payrollRuns.length > 0 ? payrollRuns.map(run => (
                ...
                >
                  <TableCell className="font-bold text-gray-700 h-16">{new Date(run.year, run.month - 1).toLocaleString('en-US', { month: 'short', year: 'numeric' })}</TableCell>
                  <TableCell>{getStatusBadge(run.status)}</TableCell>
                  <TableCell className="text-right font-bold text-gray-600">{run.employee_count}</TableCell>
                  <TableCell className="text-right font-black text-primary-600">{formatCurrency(run.total_gross)}</TableCell>
                  ...
                  <TableCell className="text-center">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="font-black text-primary-600 group-hover:bg-white border border-transparent group-hover:border-gray-200 rounded-lg"
                      onClick={(e) => {
                        e.stopPropagation();
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12 text-gray-400 font-medium">No historical records available</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
