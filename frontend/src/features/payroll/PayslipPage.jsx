...
const PayslipPage = () => {
    ...
    const PayslipModal = ({ payslip, onClose }) => {
        ...
        return (
            <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-md flex items-center justify-center z-[100] p-4 sm:p-6 animate-in fade-in zoom-in duration-300">
                <Card className="w-full max-w-4xl max-h-[92vh] overflow-hidden bg-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] rounded-[2.5rem] border-none flex flex-col">
                    ...
                    <CardHeader className="bg-gradient-to-br from-slate-900 via-slate-800 to-primary-900 p-8 sm:p-10 text-white relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 w-48 h-48 bg-primary-500/10 rounded-full blur-3xl -ml-10 -mb-10"></div>
                    ...
                    <CardContent className="p-0 flex-1 overflow-y-auto custom-scrollbar">
                        <div className="p-8 sm:p-10 space-y-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="md:col-span-2 p-8 bg-gray-50 rounded-[2rem] border border-slate-100 flex items-center gap-6 group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Authenticated Employee</p>
                                ...
                                <div className="p-8 bg-primary-50/50 rounded-[2rem] border border-primary-100/50 flex flex-col justify-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className={`h-2.5 w-2.5 rounded-full ${payslip.payment_status === 'paid' ? 'bg-primary-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]'}`}></div>
                                    ...
                                    <p className="text-xs font-medium text-gray-400 mt-2 italic">Ref: LIF-{payslip.id}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-8 bg-primary-50/30 rounded-[2rem] border border-emerald-100/50 space-y-4 relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                                            <TrendingUp className="h-16 w-16 text-primary-600" />
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Standard Basic Salary</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency(payslip.basic_salary)}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Housing & Utility Allow.</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency((payslip.housing_allowance || 0) + (payslip.lunch_allowance || 0))}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Mobility & Transport</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency(payslip.transport_allowance || 0)}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Other Tactical Allowances</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency((payslip.medical_allowance || 0) + (payslip.other_allowances || 0))}</span>
                                        </div>
                                        <div className="pt-5 border-t-2 border-dashed border-emerald-200 mt-2 flex justify-between items-center">
                                            ...
                                            <span className="text-2xl font-black text-primary-600">{formatCurrency(payslip.gross_salary)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-8 bg-orange-50/30 rounded-[2rem] border border-orange-100/50 space-y-4 relative overflow-hidden group">
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">PAYE Statutory Tax</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency(payslip.paye_tax)}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">NSSF Pension (Employee)</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency(payslip.nssf_employee)}</span>
                                        </div>
                                        {payslip.local_service_tax > 0 && (
                                            <div className="flex justify-between items-center text-sm">
                                                <span className="text-gray-500 font-medium">Local Service Tax (LST)</span>
                                                <span className="text-gray-900 font-bold">{formatCurrency(payslip.local_service_tax)}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Financial Obligations / Loans</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency((payslip.loan_deduction || 0) + (payslip.advance_deduction || 0))}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="text-gray-500 font-medium">Internal Surcharges / Misc</span>
                                            <span className="text-gray-900 font-bold">{formatCurrency(payslip.other_deductions || 0)}</span>
                                        </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-8 bg-gray-50 p-6 rounded-[2rem] border border-slate-100">
                                <div className="h-24 w-24 bg-white p-2 rounded-2xl shadow-inner border border-slate-100 flex items-center justify-center shrink-0">
                                    ...
                                    <div className="w-full h-full bg-gray-50 rounded-lg border border-dashed border-gray-200 flex flex-col items-center justify-center gap-1">
                                        <div className="w-8 h-8 opacity-10"><DollarSign /></div>
                                        <span className="text-[6px] font-bold text-slate-300">SCANNABLE QR</span>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 font-mono">Digital Signature Hash</p>
                                    <p className="text-[8px] font-mono text-gray-400 break-all leading-tight">
                    ...
                    <div className="p-8 sm:p-10 bg-gray-50 border-t border-slate-100 flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 flex flex-col">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 italic">Net Disbursement</p>
                        ...
                        <div className="flex gap-4">
                            <Button
                                ...
                                className="h-14 px-8 rounded-2xl bg-gray-900 text-white hover:bg-gray-800 gap-3 font-bold text-base shadow-xl shadow-slate-200 transition-all active:scale-95 group"
                            >
                            ...
                            <Button
                                ...
                                disabled={isEmailing}
                                variant="outline"
                                className="h-14 px-8 rounded-2xl border-2 border-gray-200 hover:border-primary-500 hover:bg-white text-gray-700 hover:text-primary-700 gap-3 font-bold text-base transition-all active:scale-95 group"
                            >
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 pb-2">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-gray-900 leading-none">Payslip Audit</h1>
                    <p className="text-gray-500 mt-3 font-medium text-lg">Centralized monitoring and dispatch of organizational pay records.</p>
                </div>
                    ...
                    <div className="flex items-center gap-3 bg-white p-2.5 rounded-[1.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 pr-5">
                        <div className="flex items-center gap-2 pl-3 border-r border-slate-100 pr-5">
                            ...
                            <select
                                ...
                                className="text-sm font-black text-gray-700 bg-transparent border-none focus:ring-0 cursor-pointer appearance-none pr-6"
                            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="p-8 bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-50 relative overflow-hidden group hover:border-emerald-200 transition-all duration-500">
                    ...
                    <div className="relative">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Monthly Gross Liability</p>
                        <h4 className="text-2xl font-black text-gray-900 group-hover:text-primary-600 transition-colors">{formatCurrency(stats.totalGross)}</h4>
                    </div>
                </div>
                ...
                <div className="p-8 bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-50 relative overflow-hidden group hover:border-primary-200 transition-all duration-500">
                    ...
                    <div className="relative">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Total Net Disbursement</p>
                        <h4 className="text-2xl font-black text-gray-900 group-hover:text-primary-600 transition-colors">{formatCurrency(stats.totalNet)}</h4>
                    </div>
                </div>

                <div className="p-8 bg-gray-900 rounded-[2rem] shadow-xl shadow-slate-300 relative overflow-hidden group">
                    ...
                    <div className="relative">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1 italic">Average Employee Net</p>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="hidden lg:flex items-center gap-2">
                    ...
                    <Badge className="bg-gray-100 text-gray-500 border-none rounded-xl px-4 py-2 cursor-pointer hover:bg-slate-200 transition-colors">Pending</Badge>
                </div>
            </div>

            {/* Main Data Table */}
            <Card className="border-none shadow-[0_24px_48px_-12px_rgba(0,0,0,0.06)] rounded-[2.5rem] overflow-hidden bg-white">
                <CardHeader className="bg-gray-50/70 border-b border-slate-100 p-10">
                    <div className="flex items-center justify-between">
                        <CardTitle className="text-xs font-black uppercase tracking-[0.3em] text-gray-400 flex items-center gap-3">
                            <div className="h-2 w-2 rounded-full bg-primary-500"></div>
                            Transactional History Logs
                        </CardTitle>
                        <div className="text-[10px] font-bold text-gray-500 bg-white px-4 py-2 rounded-xl border border-slate-100">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-gray-50/30">
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-500 uppercase tracking-[0.15em]">Personnel Identity</TableHead>
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-500 uppercase tracking-[0.15em] text-right">Gross</TableHead>
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-500 uppercase tracking-[0.15em] text-right">Deductions</TableHead>
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-900 uppercase tracking-[0.15em] text-right">Net Salary</TableHead>
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-500 uppercase tracking-[0.15em] text-center">Status</TableHead>
                                    <TableHead className="px-10 py-6 text-[11px] font-black text-gray-500 uppercase tracking-[0.15em] text-right">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-96 text-center">
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <div className="mt-4">
                                                    <p className="text-sm font-black text-gray-400 uppercase tracking-[0.4em] animate-pulse">Syncing Database Assets</p>
                                                    <p className="text-[10px] text-gray-400 font-medium mt-1">Please keep this window active...</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                    ...
                                    filteredPayslips.map((payslip) => (
                                        <TableRow key={payslip.id} className="group hover:bg-primary-50/30 transition-all duration-300 border-b border-slate-50 last:border-none">
                                            <TableCell className="px-10 py-8">
                                                <div className="flex items-center gap-5">
                                                    <div className="h-14 w-14 rounded-[1.25rem] bg-gray-900 text-white flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 group-hover:shadow-2xl group-hover:shadow-slate-900/30">
                                                        <User className="h-6 w-6" />
                                                    </div>
                                                    <div>
                                                        <p className="text-base text-gray-900 font-black group-hover:text-primary-600 transition-colors">{payslip.employee_name || payslip.employee?.full_name}</p>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider bg-gray-100 px-2 py-0.5 rounded-lg border border-slate-100 group-hover:border-primary-100 transition-colors">
                                                                {payslip.employee_number || payslip.employee?.employee_id || 'N/A'}
                                                            </span>
                                                            <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{payslip.department || payslip.employee?.department_name || 'Corporate'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell className="px-10 py-8 text-right font-bold text-gray-600">
                                                {formatCurrency(payslip.gross_salary)}
                                            </TableCell>
                                            <TableCell className="px-10 py-8 text-right font-bold text-orange-500">
                                                -{formatCurrency(payslip.total_deductions)}
                                            </TableCell>
                                            <TableCell className="px-10 py-8 text-right font-black text-primary-600 text-xl group-hover:scale-105 transition-transform duration-300">
                                                {formatCurrency(payslip.net_salary)}
                                            </TableCell>
                                            <TableCell className="px-10 py-8 text-center">
                                                <Badge className={`${payslip.payment_status === 'paid' ? 'bg-primary-50 text-primary-600 ring-1 ring-emerald-100' : 'bg-orange-50 text-orange-600 ring-1 ring-orange-100'} border-none rounded-xl px-5 py-2 text-[10px] font-black uppercase tracking-widest shadow-sm`}>
                                                    {payslip.payment_status || (payslip.is_paid ? 'Paid' : 'Pending')}
                                                </Badge>
                                            </TableCell>
                                            <TableCell className="px-10 py-8">
                                                <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                                    <Button
                                                        ...
                                                        className="h-12 w-12 rounded-2xl text-gray-400 hover:text-primary-600 hover:bg-white shadow-none hover:shadow-xl transition-all"
                                                    >
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-96 text-center">
                                            <div className="flex flex-col items-center gap-6">
                                                <div className="h-24 w-24 bg-gray-50 rounded-[2.5rem] flex items-center justify-center group shadow-inner">
                                                    <FileText className="h-10 w-10 text-slate-200 group-hover:scale-110 transition-transform" />
                                                </div>
                                                <div className="max-w-[280px]">
                                                    <p className="text-lg font-black text-gray-900 uppercase tracking-widest">Archive Vault Empty</p>
                                                    <p className="text-sm text-gray-400 mt-2 font-medium">No transactional payslip records found for the selected criteria. Try adjusting your temporal filters.</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                    </TableRow>
