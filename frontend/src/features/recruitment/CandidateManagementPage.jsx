    ...
    return (
        <div className="space-y-10 pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Recruitment</h1>
                    <p className="text-gray-500 mt-2">Manage your talent pipeline and candidate relationships.</p>
                </div>
                    ...
                    <DialogContent className="max-w-2xl bg-white rounded-3xl p-0 overflow-hidden shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto max-h-[70vh]">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700">Resume Upload</label>
                                <div className={cn(
                                    "border-2 border-dashed rounded-2xl p-8 transition-all group flex flex-col items-center justify-center text-center cursor-pointer hover:border-emerald-500 hover:bg-primary-50/30",
                                    formData.resume ? 'border-emerald-500 bg-primary-50/30' : 'border-gray-200 bg-gray-50'
                                )}>
                                    ...
                                    <label htmlFor="resume-upload" className="cursor-pointer space-y-3 w-full">
                                        {isParsing ? (
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <p className="text-sm font-medium text-primary-600">Parsing resume details...</p>
                                            </div>
                                        ) : (
                                            <>
                                                <div className="h-12 w-12 rounded-xl bg-white shadow-sm flex items-center justify-center mx-auto text-gray-400 group-hover:text-primary-500 transition-colors border border-slate-100">
                            ...
                            <div className="bg-gray-50 p-6 rounded-2xl space-y-2">
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Additional Info</p>
                                <div className="grid grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <StatBox label="Offer Sent" value={stats.offer} icon={Mail} color="bg-primary-50 text-primary-600" />
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="w-full md:w-48">
                    <select
                        ...
                        className="w-full h-10 px-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-200 outline-none"
                    >
...
const CandidateCard = ({ candidate, onClick }) => (
    <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group">
        <Card className="rounded-3xl border border-gray-200 bg-white shadow-sm hover:shadow-xl hover:border-gray-300 transition-all cursor-pointer h-full flex flex-col" onClick={onClick}>
            <div className="p-6 pb-4 flex items-start justify-between">
                <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-2xl bg-gray-100 flex items-center justify-center overflow-hidden border border-slate-100">
                        {candidate.photo ? (
                    ...
                    <div>
                        <h3 className="font-bold text-gray-900 group-hover:text-primary-600 transition-colors">{candidate.first_name} {candidate.last_name}</h3>
            ...
            <div className="px-6 py-4 mt-auto">
                <div className="flex flex-wrap gap-2 mb-4 h-6 overflow-hidden">
                    {candidate.skills?.slice(0, 3).map((skill, idx) => (
                        <span key={idx} className="px-2 py-0.5 bg-gray-50 text-gray-600 text-xs font-medium rounded-md border border-slate-100">
                            {skill}
                        </span>
                    ))}
                </div>
                <Button variant="outline" className="w-full rounded-xl border-gray-200 hover:bg-gray-50 text-gray-600 h-10 text-sm font-medium">
                    View Profile
                </Button>
            </div>
        </Card>
