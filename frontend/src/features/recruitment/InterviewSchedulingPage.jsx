    ...
    return (
        <div className="space-y-10 pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Interview Schedule</h1>
                    <p className="text-slate-500 mt-2">Manage upcoming interviews and assessments.</p>
                </div>
                <Button onClick={() => setShowForm(true)} className="rounded-xl h-11 bg-slate-900 text-white font-medium shadow-lg shadow-slate-900/20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                <div className="space-y-6">
                    {/* Calendar Widget */}
                    <Card className="rounded-3xl border border-slate-200 shadow-sm bg-white overflow-hidden p-6">
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className="w-full mt-2 h-12 px-4 bg-slate-50 border border-slate-100 rounded-xl font-medium text-slate-900 focus:ring-2 focus:ring-slate-200 outline-none"
                            />
                        </div>
                        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-center">
                            <p className="text-3xl font-bold text-slate-900">{todayInterviews.length}</p>
                            <p className="text-sm font-medium text-slate-500">Sessions on {new Date(selectedDate).toLocaleDateString()}</p>
                        </div>
                    </Card>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm text-center">
                            ...
                            <p className="text-lg font-bold text-slate-900">{todayInterviews.filter(i => i.interview_type === 'video_call').length}</p>
                            <p className="text-xs text-slate-500 font-medium">Video Calls</p>
                        </div>
                        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm text-center">
                            <MapPin className="h-5 w-5 text-emerald-500 mx-auto mb-2" />
                            <p className="text-lg font-bold text-slate-900">{todayInterviews.filter(i => i.interview_type === 'in_person').length}</p>
                            <p className="text-xs text-slate-500 font-medium">In Person</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    {/* Interview List */}
                    <Card className="rounded-3xl border border-slate-200 shadow-sm bg-white min-h-[500px]">
                        ...
                        <div className="p-6 space-y-4">
                            {isLoading ? (
                                ...
                                <div className="text-center py-20 text-slate-400">
                                    ...
                                    <p className="font-medium text-slate-600">No interviews scheduled</p>
                                    <p className="text-sm">Select a different date or schedule a new one.</p>
                                </div>
                            ) : (
                                todayInterviews.map((interview) => (
                                    <div key={interview.id} className="group flex flex-col sm:flex-row items-center gap-6 p-6 bg-white border border-slate-100 rounded-2xl hover:shadow-md hover:border-slate-200 transition-all">
                                        <div className="flex flex-col items-center justify-center h-16 w-16 bg-slate-50 rounded-xl text-slate-900 font-bold shrink-0">
                                            <span className="text-lg">{interview.scheduled_time}</span>
                                        </div>

                                        <div className="flex-1 text-center sm:text-left">
                                            <h3 className="font-bold text-slate-900 text-lg">{interview.candidate_name}</h3>
                                            <div className="flex flex-wrap justify-center sm:justify-start gap-4 text-sm text-slate-500 mt-1">
                ...
                <DialogContent className="max-w-2xl bg-white rounded-3xl p-0 overflow-hidden shadow-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-6">
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700">Interviewer</label>
                                <select
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <select name="duration_minutes" value={formData.duration_minutes} onChange={handleInputChange} className="w-full h-10 px-3 bg-white border border-slate-200 rounded-md text-sm outline-none">
                                ...
                                {interviewTypes.map((type) => {
                                    ...
                                    return (
                                        <label key={type.value} className={cn(
                                            ...
                                            formData.interview_type === type.value ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200 hover:bg-slate-50'
                                        )}>
                                            ...
                                            <Icon className={cn("h-5 w-5", formData.interview_type === type.value ? 'text-white' : 'text-slate-500')} />
                                            <div className="flex flex-col">
                                                <span className="text-sm font-bold">{type.label}</span>
                                                <span className={cn("text-xs", formData.interview_type === type.value ? 'text-slate-300' : 'text-slate-400')}>{type.desc}</span>
                                            </div>
                                        </label>
                                    );
                                })}
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-slate-700">{formData.interview_type === 'video_call' ? 'Meeting Link' : 'Location'}</label>
                            <Input
